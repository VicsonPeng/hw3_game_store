...
class GarticClient:
    ...
    def setup_ui(self):
        # === 主佈局 (Top: Info, Left: Tool, Center: Canvas, Right: Chat/Rank) ===
        ...
        self.lbl_timer = tk.Label(top_frame, text="60", font=("Arial", 24, "bold"), bg="#333", fg="#ff5555")
        ...
        # 3. 左側工具列 (只有自己是畫家時可用，但我們用 disable 控制)
    ...
    def process_packet(self, line):
        ...
        try:
            ...
            if mtype == "SYS_MSG":
                ...
                if not self.is_drawer:
                    self.lbl_word.config(text=data) # 更新遮罩字串
    ...
    def prompt_selection(self, words):
        # 使用簡單的對話框選詞，這裡需要用到 main thread，所以用 after
        def ask():
            choice = SimpleChoiceDialog(self.root, "選擇題目", "請選擇難度:", words).result
            if choice is not None:
                msg = json.dumps({"type": "SELECT_WORD", "data": choice}) + "\n"
    ...
    def update_rank(self, players):
        ...
        for i, p in enumerate(players):
            icon = "✏️" if p['is_drawer'] else f"{i+1}."
...
class SimpleChoiceDialog(tk.Toplevel):
    def __init__(self, parent, title, prompt, options):
        super().__init__(parent)
        self.title(title)
        self.result = None
        
        tk.Label(self, text=prompt, font=("Arial", 12)).pack(pady=10)
        
        btn_frame = tk.Frame(self)
        btn_frame.pack(pady=10)
        
        for i, opt in enumerate(options):
            tk.Button(btn_frame, text=opt, font=("Arial", 11), width=10,
                      command=lambda idx=i: self.set_result(idx)).pack(side=tk.LEFT, padx=5)
