...
class GarticClient:
    ...
    def setup_ui(self):
        ...
        self.lbl_word = tk.Label(top_frame, text="連線中...", font=("Microsoft JhengHei", 20, "bold"), bg="#333", fg="white")
        ...
        self.chat_log.tag_config("correct", foreground="green", font=("Microsoft JhengHei", 10, "bold"))
        ...
        # 3. 左側工具列
        self.tool_frame = tk.Frame(self.root, bg="#444", width=50)
    ...
    def process_packet(self, line):
        ...
        try:
            ...
            if mtype == "FORCE_QUIT":
                messagebox.showinfo("遊戲結束", "遊戲結束！視窗將關閉。")
                self.running = False
                self.root.quit()
                return
    ...
    def prompt_selection(self, words):
        def ask():
            ...
            top.grab_set() 
            Label(top, text="請選擇要畫的題目:", font=("Microsoft JhengHei", 12)).pack(pady=10)
            def select(idx):
    ...
    def update_rank(self, players):
        ...
        for i, p in enumerate(players):
            icon = "✎" if p.get('is_drawer') else f"#{i+1}"
            # 格式優化： #1  Name  (Score分)
            self.rank_list.insert(tk.END, f" {icon}  {p['name']}  ({p['score']}分)")
